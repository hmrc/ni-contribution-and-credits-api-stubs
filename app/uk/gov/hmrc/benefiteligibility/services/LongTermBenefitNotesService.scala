...
class LongTermBenefitNotesService @Inject() (jsonUtils: JsonUtils) {
  ...
  def mapIdentifierToResponse(
    ...
    StubId.withName(identifier) match {
      ...
      case AA000002A | AA000002 | AA000003 | AA000004 | AA000007 =>
        Future.successful(
          Ok(
            jsonUtils.readJsonFile(
              s"conf/resources/data/jsons/longTermBenefitNotes/SuccessResponse.json"
            )
          )
        )
      case AA000005 =>
        Future.successful(
          Ok(
            jsonUtils.readJsonFile(
              s"conf/resources/data/jsons/longTermBenefitNotes/MinimalSuccessResponse.json"
            )
          )
        )
      case AA000006 =>
