...
import uk.gov.hmrc.benefiteligibility.services.StubId.{AA000001A, AA000002A, AA000005, AA000006}
    ...
    StubId.withName(identifier) match {
      ...
      case _ =>
        Future.successful(
          BadRequest(
            jsonUtils.readJsonFile(
              s"conf/resources/data/jsons/benefitSchemeDetails/ErrorResponseDefault.json"
            )
          )
        )
    }
