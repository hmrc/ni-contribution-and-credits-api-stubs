...
class LongTermBenefitCalculationDetailsService @Inject() (jsonUtils: JsonUtils) {
    ...
    StubId.withName(identifier) match {
      ...
      case AA000005 =>
        Future.successful(
          Ok(
            jsonUtils.readJsonFile(
              s"conf/resources/data/jsons/longTermBenefitCalcDetails/MinimalSuccessResponse.json"
            )
          )
        )
      case AA000006 =>
